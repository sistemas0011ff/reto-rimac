"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ChileCompletionRules = void 0;
class ChileCompletionRules {
    apply(appointment) {
        return __awaiter(this, void 0, void 0, function* () {
            console.log('Aplicando reglas de completación para Chile');
            // Notificar a FONASA
            yield this.notifyFONASA(appointment);
            // Actualizar sistema de salud chileno
            yield this.updateChileHealthSystem(appointment);
        });
    }
    notifyFONASA(appointment) {
        return __awaiter(this, void 0, void 0, function* () {
            // Implementación de notificación a FONASA
            console.log('Notificando a FONASA:', appointment.id);
        });
    }
    updateChileHealthSystem(appointment) {
        return __awaiter(this, void 0, void 0, function* () {
            // Implementación de actualización del sistema
            console.log('Actualizando sistema de salud chileno');
        });
    }
}
exports.ChileCompletionRules = ChileCompletionRules;
