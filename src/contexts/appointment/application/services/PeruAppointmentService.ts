 ...
 */
export class PeruAppointmentService implements ICountryAppointmentService {
    private processedCount: number = 0;
    private lastProcessedAt: string = '';
    
    constructor(
        private peruProcessor: ICountryAppointmentProcessor
    ) {}
    ...
    async processMessage(messageBody: SNSMessageBody): Promise<void> {
        try {
            ...
            await this.applyPeruSpecificLogic(appointmentData);
            
            // Procesar la cita en el sistema de Perú (MySQL)
            await this.peruProcessor.processAppointment(appointmentData);
            
            // Enviar confirmación a EventBridge
            const eventId = await this.peruProcessor.sendConfirmation(appointmentData);
     ...
     */
    private validatePeruAppointment(appointmentData: AppointmentData): void {
     ...
     */
    private async applyPeruSpecificLogic(appointmentData: AppointmentData): Promise<void> {
        ...
        try {
            ...
            await this.validatePeruBusinessHours(appointmentData);
            
            // Lógica específica 2: Aplicar reglas de asegurados peruanos
            await this.applyPeruInsuranceRules(appointmentData);
            
            // Lógica específica 3: Verificar disponibilidad de especialistas en Perú
            await this.checkPeruSpecialistAvailability(appointmentData);
     ...
     * @param appointmentData Datos de la cita
     */
    private async validatePeruBusinessHours(appointmentData: AppointmentData): Promise<void> {
     ...
     * @param appointmentData Datos de la cita
     */
    private async applyPeruInsuranceRules(appointmentData: AppointmentData): Promise<void> {
        ...
        switch (insuranceType) {
            case 'EsSalud':
                await this.validateEsSaludRules(appointmentData);
                break;
            case 'SIS':
                await this.validateSISRules(appointmentData);
                break;
            case 'Privado':
     ...
     */
    private async validateEsSaludRules(appointmentData: AppointmentData): Promise<void> {
     ...
     */
    private async validateSISRules(appointmentData: AppointmentData): Promise<void> {
     ...
     * @param appointmentData Datos de la cita
     */
    private async checkPeruSpecialistAvailability(appointmentData: AppointmentData): Promise<void> {
